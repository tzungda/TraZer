...
void TW_CALL SetEnableShadowmapCB(const void *value, void *clientData)
{
	...
	glUseProgram(blinPhongShaderProgram);
...
void resizeGbuffers(int width, int height)
{
...
void setupGUI()
{
	...
	{
		TwEnumVal gBufferTypeEV[6] = { { -1, "Forward" },{ 0, "Combined" },{ 1, "Position" },{ 2, "Normal" },{ 3, "Diffuse Color" },{ 4, "Depth" } };
		TwType type = TwDefineEnum("bufferType", gBufferTypeEV, 6);
...
void setupShaders()
{
	string dirPath = "13.3.deferred_shading/";

	blinPhongShaderProgram = loadShader(dirPath + "blinnPhong.vs", dirPath + "blinnPhong.fs");
	{
		glUseProgram(blinPhongShaderProgram);

		//Cache uniform variable id
		blinnPhongShaderUniforms.lightPos = glGetUniformLocation(blinPhongShaderProgram, "lightPos");
		blinnPhongShaderUniforms.viewPos = glGetUniformLocation(blinPhongShaderProgram, "viewPos");
		blinnPhongShaderUniforms.model_matrix = glGetUniformLocation(blinPhongShaderProgram, "model");
		blinnPhongShaderUniforms.view_matrix = glGetUniformLocation(blinPhongShaderProgram, "view");
		blinnPhongShaderUniforms.projection_matrix = glGetUniformLocation(blinPhongShaderProgram, "projection");
		blinnPhongShaderUniforms.light_matrix = glGetUniformLocation(blinPhongShaderProgram, "lightSpaceMatrix");
		blinnPhongShaderUniforms.useShadowmap = glGetUniformLocation(blinPhongShaderProgram, "useShadowMap");
		blinnPhongShaderUniforms.useBias = glGetUniformLocation(blinPhongShaderProgram, "useBias");
		blinnPhongShaderUniforms.pcfKernel = glGetUniformLocation(blinPhongShaderProgram, "pcfKernel");

		glUniform1i(glGetUniformLocation(blinPhongShaderProgram, "diffuseTexture"), 0);
		glUniform1i(glGetUniformLocation(blinPhongShaderProgram, "shadowMap"), 1);
	}

	lightShaderProgram = loadShader(dirPath + "shadowmapping-light.vs", dirPath + "shadowmapping-light.fs");
	{
		glUseProgram(lightShaderProgram);

		//Cache uniform variable id
		lightShaderUniforms.model_matrix = glGetUniformLocation(lightShaderProgram, "model");
		lightShaderUniforms.light_matrix = glGetUniformLocation(lightShaderProgram, "lightSpaceMatrix");

		glUniform1i(glGetUniformLocation(lightShaderProgram, "diffuseTexture"), 0);
	}

	gBufferShaderProgram = loadShader(dirPath + "G-Buffer.vs", dirPath + "G-Buffer.fs");
	{
	...
	gBufferVisualizationShaderProgram = loadShader(dirPath + "G-Buffer-Visualization.vs", dirPath + "G-Buffer-Visualization.fs");
	{
		glUseProgram(gBufferVisualizationShaderProgram);
	...
	defferedShadingShaderProgram = loadShader(dirPath + "Deffered-Shading.vs", dirPath + "Deffered-Shading.fs");
	{
		glUseProgram(defferedShadingShaderProgram);

		//Cache uniform variable id
		defferedShadingShaderUniforms.viewPos = glGetUniformLocation(gBufferShaderProgram, "viewPos");
		for (int i = 0; i < NUM_LIGHTS; ++i)
		{
			defferedShadingShaderUniforms.lightPositions[i] = glGetUniformLocation(defferedShadingShaderProgram, string("lights[" + to_string(i) + "].Position").c_str());
			defferedShadingShaderUniforms.lightColors[i] = glGetUniformLocation(defferedShadingShaderProgram, string("lights[" + to_string(i) + "].Color").c_str());
		}

		glUniform1i(glGetUniformLocation(defferedShadingShaderProgram, "gPosition"), 0);
		glUniform1i(glGetUniformLocation(defferedShadingShaderProgram, "gNormal"), 1);
		glUniform1i(glGetUniformLocation(defferedShadingShaderProgram, "gDiffuse"), 2);
	}
}
void setupGbuffers()
{
...
void My_Display()
{
	...
	glUseProgram(lightShaderProgram);
	{
		glUniformMatrix4fv(lightShaderUniforms.light_matrix, 1, GL_FALSE, value_ptr(lightSpaceMatrix));
		...
		for (int i = 0; i < m_shapes.size(); ++i)
		{
			...
			glBindVertexArray(m_shape.vao);
			glUniformMatrix4fv(lightShaderUniforms.model_matrix, 1, GL_FALSE, value_ptr(m_shape.getTransformationMatrix()));
	...
	if (m_G_Buffer_Type == -1)
	{
		...
		glUseProgram(blinPhongShaderProgram);
		{
			...
			for (int i = 0; i < m_shapes.size(); ++i)
			{
				...
				glActiveTexture(GL_TEXTURE1); glBindTexture(GL_TEXTURE_2D, m_shadowDepthMap);
				glDrawElements(GL_TRIANGLES, m_shape.indexCount, GL_UNSIGNED_INT, 0);
				glBindVertexArray(0);
			}
		}
	...
	else if (m_G_Buffer_Type == 0)
	{
		glUseProgram(defferedShadingShaderProgram);
		{
			glUniform3fv(defferedShadingShaderUniforms.viewPos, 1, value_ptr(m_camera.GetWorldEyePosition()));

			for (int i = 0; i < m_lightPositions.size(); ++i)
			{
				glUniform3fv(defferedShadingShaderUniforms.lightPositions[i], 1, value_ptr(m_lightPositions[i]));
				glUniform3fv(defferedShadingShaderUniforms.lightColors[i], 1, value_ptr(m_lightColors[i]));
			}
...
int main(int argc, char *argv[])
{
	...
	m_mainWindow = glutCreateWindow("13-3. Deferred Shading"); // You cannot use OpenGL functions before this line; The OpenGL context must be created first by glutCreateWindow()!
#ifdef _MSC_VER
	glewInit();
#endif
