...
private:
	...
	std::map<std::string, std::shared_ptr<tzCoreTexture>>		mTextureList; // attribute(eg. diffuse, ambient) and the correspond texture
...
public:
	...
	*/
	...
	void											addTexture( const std::string &name, std::shared_ptr<tzCoreTexture> texture );
